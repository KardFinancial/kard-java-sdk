                    ...
                    CreateContentStrategyAttributes
                        ...
                        .filters(
                            ContentStrategyFilters
                                ...
                                .offerFeatures(
                                    Optional.of(
                                        Arrays.asList(OfferFeatures.INTERACTIVE)
                                    )
                                )
                                .build()
                        )
...
client.organizations().placements().create(
    ...
    CreatePlacementRequestBody
        .builder()
        .data(
            CreatePlacementDataUnion.placement(
...
**filterType:** `Optional<PlacementTypeFilter>` — Filter by placement type (placement, placementPushNotification, placementEmail, placementBatchActivation, or placementGroup)
...
client.organizations().placements().update(
    ...
    UpdatePlacementRequestBody
        .builder()
        .data(
            UpdatePlacementDataUnion.placement(
...
**filterRange:** `Optional<EarnedRewardsRange>` — Time window for the returned transactions, ending now. Supported values are `12M`, `6M`, `3M`, and `YTD` (since January 1 of the current year). Defaults to `12M` when omitted. Also scopes `lifetimeRewardsInCents`, so the meta total always matches the returned rows.
...
**filterSearch:** `Optional<String>` — Case-insensitive substring search. Returns offers whose offer name or category name contains the search string.
...
**filterCity:** `Optional<String>` — Case-insensitive substring match on the location's city. Never defines the search area; applied as an additional constraint alongside a radius search when `filter[latitude]`/`filter[longitude]`/`filter[radius]` are also provided.
...
**filterZipCode:** `Optional<String>` — Exact-match filter on the location's zip code. Never defines the search area; applied as an additional constraint alongside a radius search when `filter[latitude]`/`filter[longitude]`/`filter[radius]` are also provided.
...
**filterState:** `Optional<State>` — Exact-match filter on the location's state. Never defines the search area; applied as an additional constraint alongside a radius search when `filter[latitude]`/`filter[longitude]`/`filter[radius]` are also provided.
...
**filterLongitude:** `Optional<Double>` — Longitude of the point to search around. Must be provided together with `filter[latitude]`; combine with `filter[radius]` to run a radius search.
...
**filterLatitude:** `Optional<Double>` — Latitude of the point to search around. Must be provided together with `filter[longitude]`; combine with `filter[radius]` to run a radius search.
...
**filterRadius:** `Optional<Integer>` — Radius in miles to search around the point given by `filter[latitude]`/`filter[longitude]` (default 10, minimum 1). Has no effect unless both latitude and longitude are also provided — it is ignored when only `filter[zipCode]`, `filter[city]`, or `filter[state]` is used, without lat/long.
