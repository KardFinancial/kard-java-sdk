/**
 * This file was auto-generated by Fern from our API Definition.
 */
package com.kard.api.resources.transactions.types;

import com.fasterxml.jackson.annotation.JsonAnyGetter;
import com.fasterxml.jackson.annotation.JsonAnySetter;
import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonSetter;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.kard.api.core.ObjectMappers;
import java.util.HashMap;
import java.util.Map;
import java.util.Objects;
import org.jetbrains.annotations.NotNull;

@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(builder = TransactionsRequest.Builder.class)
public final class TransactionsRequest {
    private final String id;

    private final TransactionsAttributes attributes;

    private final Map<String, Object> additionalProperties;

    private TransactionsRequest(
            String id, TransactionsAttributes attributes, Map<String, Object> additionalProperties) {
        this.id = id;
        this.attributes = attributes;
        this.additionalProperties = additionalProperties;
    }

    /**
     * @return Unique identifier for the transaction event. This <b>must</b> be unique for each distinct event sent to the API.
     */
    @JsonProperty("id")
    public String getId() {
        return id;
    }

    @JsonProperty("attributes")
    public TransactionsAttributes getAttributes() {
        return attributes;
    }

    @java.lang.Override
    public boolean equals(Object other) {
        if (this == other) return true;
        return other instanceof TransactionsRequest && equalTo((TransactionsRequest) other);
    }

    @JsonAnyGetter
    public Map<String, Object> getAdditionalProperties() {
        return this.additionalProperties;
    }

    private boolean equalTo(TransactionsRequest other) {
        return id.equals(other.id) && attributes.equals(other.attributes);
    }

    @java.lang.Override
    public int hashCode() {
        return Objects.hash(this.id, this.attributes);
    }

    @java.lang.Override
    public String toString() {
        return ObjectMappers.stringify(this);
    }

    public static IdStage builder() {
        return new Builder();
    }

    public interface IdStage {
        /**
         * <p>Unique identifier for the transaction event. This <b>must</b> be unique for each distinct event sent to the API.</p>
         */
        AttributesStage id(@NotNull String id);

        Builder from(TransactionsRequest other);
    }

    public interface AttributesStage {
        _FinalStage attributes(@NotNull TransactionsAttributes attributes);
    }

    public interface _FinalStage {
        TransactionsRequest build();
    }

    @JsonIgnoreProperties(ignoreUnknown = true)
    public static final class Builder implements IdStage, AttributesStage, _FinalStage {
        private String id;

        private TransactionsAttributes attributes;

        @JsonAnySetter
        private Map<String, Object> additionalProperties = new HashMap<>();

        private Builder() {}

        @java.lang.Override
        public Builder from(TransactionsRequest other) {
            id(other.getId());
            attributes(other.getAttributes());
            return this;
        }

        /**
         * <p>Unique identifier for the transaction event. This <b>must</b> be unique for each distinct event sent to the API.</p>
         * <p>Unique identifier for the transaction event. This <b>must</b> be unique for each distinct event sent to the API.</p>
         * @return Reference to {@code this} so that method calls can be chained together.
         */
        @java.lang.Override
        @JsonSetter("id")
        public AttributesStage id(@NotNull String id) {
            this.id = Objects.requireNonNull(id, "id must not be null");
            return this;
        }

        @java.lang.Override
        @JsonSetter("attributes")
        public _FinalStage attributes(@NotNull TransactionsAttributes attributes) {
            this.attributes = Objects.requireNonNull(attributes, "attributes must not be null");
            return this;
        }

        @java.lang.Override
        public TransactionsRequest build() {
            return new TransactionsRequest(id, attributes, additionalProperties);
        }
    }
}
